...
const patientSafetyThreads: PatientConversation[] = [
  {
    id: 't1',
    patientName: 'David T.',
    text: 'Daily check-in: Nausea within a few hours of Arm A dosing, ongoing since Tuesday. Discussed symptom intensity vs. expected AE profile and whether to hold dose or add supportive care.',
    time: '2 min ago',
  },
  {
    id: 't2',
    patientName: 'Maria G.',
    text: 'Daily check-in: Mild GI upset week one of current cycle. Reviewed meal timing and hydration with site; aligned on watch-and-wait unless vomiting or weight loss.',
    time: '8 min ago',
  },
  {
    id: 't3',
    patientName: 'Thomas O.',
    text: 'Daily check-in: Higher dose cohort — fatigue layered on nausea. Trial discussion: prior schedule change for another participant; explored moving PRO window to afternoons for earlier symptom capture.',
    time: '22 min ago',
  },
  {
    id: 't4',
    patientName: 'Robert K.',
    text: 'Daily check-in: Fatigue stable, nausea improved vs. last visit. Discussed shifting remote check-ins to afternoons; patient reports better diary completion and same-day CRC follow-up.',
    time: '35 min ago',
  },
  {
    id: 't5',
    patientName: 'Jennifer W.',
    text: 'Daily check-in: Missed two e-diary entries due to work travel. Discussed protocol-acceptable backfill in Pulse vs. site call-in; documented reason in source.',
    time: '1 hr ago',
  },
  {
    id: 't6',
    patientName: 'David T.',
    text: 'Follow-up summary: Used “add note for yesterday” after missed diary; CRC acknowledged same day and updated visit worksheet. Reinforced escalation path if nausea worsens.',
    time: '1 hr ago',
  },
]
...
function App() {
  ...
  const [trialsList, setTrialsList] = useState<ClinicalTrialCard[]>(clinicalTrialCards)

  const filteredHomeTrials = useMemo(
    () => trialsList.filter((card) => matchesTrialSearch(card, trialSearchQuery)),
    [trialSearchQuery, trialsList],
  )

  const status = useMemo(() => {
    const recruiting = trialsList.filter((c) => c.trialStatusBadge === 'Recruiting').length
    const enrolledApprox = 48 * trialsList.length
    return {
      activeTrials: trialsList.length,
  ...
  return (
    <div className={`app-container ${darkMode ? 'dark' : ''}`}>
      <div className="app-header">
        ...
        {activeTab === 'overview' && (
          <button className="messages-btn" onClick={() => setMessagesOpen(true)} title="Open check-in summaries">
      ...
      <div className="app-content">
        ...
        <div className="projects-section">
          {activeTab === 'analytics' ? (
            <CohortAnalytics
              ...
              trials={trialsList}
              onAddTrial={(newTrial) => {
                setTrialsList(prev => [newTrial, ...prev])
                // We do not auto-select the new trial
              }}
            />
          ) : activeTab === 'calendar' ? (
        ...
        {activeTab === 'overview' && (
          <div className={`messages-section ${messagesOpen ? 'show' : ''}`}>
            <button className="messages-close" onClick={() => setMessagesOpen(false)} title="Close check-in summaries">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
